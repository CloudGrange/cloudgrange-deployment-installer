# FUTURE OPTIONAL PROFILE — RKE2/Kubernetes BOM installer design (not the M0/M1 product path)

> **This document is a future optional HA/Kubernetes profile. It is NOT the current product installer.**
>
> The M0/M1 product path is the Compose installer at the repo root
> (`Install-CloudGrange.ps1`, modes Online/Bundled/Appliance) — see
> [`docs/product-installer-design.md`](../product-installer-design.md) for the current,
> authoritative design. On-premises runtime is Docker Compose on a single Ubuntu VM (ADR-029);
> HA is that same VM run as a Hyper-V failover-cluster role (WSFC+CSV), not a Kubernetes cluster.
>
> Everything below this line describes a **retained-but-not-scheduled** RKE2 + per-component Helm
> chart + OpenBao-as-system-of-record design. It is kept as real qualification and design work in
> case the two-VM active/standby Compose HA option (Postgres streaming replication + keepalived)
> also proves insufficient and K3s/RKE2 is revisited — see decisions 1 and 2 in
...
> in cloudgrange-internal (link 404s until that PR merges to `main` — expected, not a defect in
> this document). Nothing below this banner is current or authoritative for M0/M1; do not treat
> section headers, phase names or "Status: authoritative" language further down as describing the
> product as shipped.

---

# CloudGrange product installer design (F12, Wave 4) — RKE2/Kubernetes profile

**Status:** was the authoritative implementation design for F12-1 (Story AB#8129, Tasks 9015–9018), the installer-side contract for F12-4 (AB#8130, Tasks 9019–9021) and M0-compact-restore (AB#8893, Tasks 9032–9034) before the 2026-09-14 reset. Written 2026-09-12 for the Wave 4 implementation agents. **Revision 2** (same day) resolves independent review `installer-5`: the site configuration now consumes the infrastructure `cg-site-config-v1` contract as-is (§1.6–§1.7), OpenBao bootstrap and the setup token are durably recoverable (§3.4 phases 6, 9, 13), migration is its own phase (phase 8), the checkpoint chain has an exact atomic write order and recovery table (§3.2), and the BOM schema ships with fixtures and a CI check (§2.3). It adds no ADO scope and closes nothing; ADO remains the status authority. **This revision history predates and is superseded by the 2026-09-14 reset — see the banner above.**
...
**Note:** this whole document is the future-optional RKE2/Kubernetes profile (see banner above). The
`installer/` checkpoint/phase/envelope/evidence engine referenced in the "Built by this design"
column below is a generic mechanism that the current Compose product installer also uses — see
[`docs/product-installer-design.md`](../product-installer-design.md) for that installer's current,
authoritative description. Everything else in this table (RKE2 runtime bring-up, per-component Helm
charts) is specific to this future-optional profile.

| Exists today (reusable) | Built by this design | Deferred (named, not silent) |
|---|---|---|
| `experiments/compact-rke2/Invoke-CompactRke2Experiment.ps1`: root PS7 on Ubuntu 24.04, SHA-256 of pinned RKE2 tarballs before extraction, request-identity checkpoint, foreign-runtime refusal, bounded readiness wait (qualified in pipelines 976–999) — **future optional profile only; not the M0/M1 runtime** | The generic phase/checkpoint/envelope/evidence installer engine under `installer/` (also the current product's engine), plus this profile's RKE2-specific phases, modes Plan/Install/Verify/Update/Rollback/Restore/Uninstall | RKE2/K3s multi-node profiles; Bundled and Appliance packaging (BOM schema already supports them); arm64 |
...
| Component Dockerfiles for api, portal, relay; agent `Install-Agent.ps1` (sc.exe, shared-token enrollment) | **Future-optional profile only:** per-component Helm charts in this repo; GHCR names `ghcr.io/cloudgrange/<component>` | PITR/WAL backup (format decided by 8892); certificate-rotation mode; OCI publication of charts (charts ship in the bundle only) |
| Legacy compose-era `Install-/Update-/Uninstall-CloudGrange.ps1` at repo root | See [`docs/product-installer-design.md`](../product-installer-design.md) — this is the current M0/M1 product installer, not part of this future-optional profile | Air-gap certification (Online cached resume only, per qualification 986) |

Nothing in this document claims a passing runtime test. Every "must" below is an acceptance obligation for the named work package, **in the future-optional RKE2/Kubernetes profile only** unless otherwise noted.
...
**Scope note:** §3.1–§3.2 and the phase-engine mechanics (checkpoint chain, envelope, resume/refusal
rules, evidence writer) are generic and also underpin the current Compose product installer — see
[`docs/product-installer-design.md`](../product-installer-design.md). Phases 3 through 11 below
(RKE2 runtime bring-up and the per-component HelmChart CRs `cg-base`/`cg-postgres`/`cg-openbao`/
`cg-keycloak`/`cg-api`/`cg-portal`/`cg-gateway`) are specific to this future-optional
RKE2/Kubernetes profile and are not run by the current product.
...
#### Phase 3 `runtime` (RKE2) — future optional profile, not the M0/M1 runtime (Compose)
...
### 5.3 Secrets: OpenBao is the system of record — future optional profile only (current default is PostgresEncryptedSecretsProvider, ADR-009)
