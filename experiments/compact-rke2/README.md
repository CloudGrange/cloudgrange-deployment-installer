...
The management topology/preflight contract was admitted through infrastructure PR3. Its owned live prerequisite gate subsequently passed in infrastructure pipeline999; Task8912 is closed. The dated qualification record and trust/budget/abort design (Tasks8907/8908) are now published through internal PR33. Parent-story reconciliation follows source admission. These results cover the lab prerequisite envelope; assembled-product recovery remains open.
...
Remaining F12 work includes fresh-install interruption, other bootstrap boundaries and torn writes, adoption of the storage/TLS/PKI prerequisite contracts and assembled product testing. Public-egress denial has been tested for cached resume of the existing node; a fresh disconnected installation is not qualified. Production recovery or HA claims are not proved by this single-node experiment. All original M0/M1 acceptance remains required.

## Source admission

The integration branch preserves the experiment source, manifest and configuration from the original branch. Seven historical commits lack author signoffs; the new signed integration commit admits the reviewed source without rewriting those historical revisions. The deployed owner remains `8819c5fd808ad5bca9dbeddc70cc6d36773dd5a3`.

The source qualification workflow replaces invalid reusable-workflow references to the former GitHub organization. It retains whole-repository PowerShell syntax and PSScriptAnalyzer error checks, adds the nine existing artifact/preflight cases, enforces author-matching DCO and retains source hashes and original results. It publishes no installer image. The obsolete automatic Docker/Trivy path targeted the historical Compose installer; a signed, scanned Compact release composition remains required under F01/F12 before product distribution.
