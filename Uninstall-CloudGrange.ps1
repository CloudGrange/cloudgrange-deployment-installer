...
if (-not $Force) {
    Write-Host ""
    Write-Host "  [WARNING] This will permanently remove all CloudGrange data." -ForegroundColor Yellow
    ...
    Write-Host "    - Virtual switch: CloudGrangeSwitch / cloudgrange-internal" -ForegroundColor Yellow
    Write-Host "    - Windows Firewall rules matching 'CloudGrange*'" -ForegroundColor Yellow
    Write-Host ""

    if ($PSCmdlet.ShouldContinue(
            "Proceed with CloudGrange uninstall? This cannot be undone.",
            "CloudGrange Uninstaller")) {
...
try {
    $cred = Get-Credential -UserName 'cloudgrange' -Message 'VM credential for container teardown'
...
# The installer creates either 'CloudGrangeSwitch' (ADR-029 appliance mode) or
# 'cloudgrange-internal' (compose/online mode).  Remove both if present.
Write-Progress-Step "Removing CloudGrange virtual switches"
foreach ($switchName in @('CloudGrangeSwitch', 'cloudgrange-internal')) {
...
# Remove the WinNAT entry that pairs with cloudgrange-internal.
Remove-NetNat -Name 'CloudGrangeNAT' -Confirm:$false -ErrorAction SilentlyContinue

# Remove the host-side static IP on the switch NIC (if still present).
$hostNic = Get-NetAdapter | Where-Object { $_.Name -match 'vEthernet.*cloudgrange' } -ErrorAction SilentlyContinue
if ($hostNic) {
...
# Match on DisplayName wildcard 'CloudGrange*' to catch any variant the installer created.
Write-Progress-Step "Removing CloudGrange firewall rules and port proxy"
$fwRules = Get-NetFirewallRule -ErrorAction SilentlyContinue |
    Where-Object { $_.DisplayName -like 'CloudGrange*' }
if ($fwRules) {
...
Write-Host ""
Write-Host "  [CloudGrange] Uninstall complete." -ForegroundColor Green
