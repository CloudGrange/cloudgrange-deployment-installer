...
do_k3s_installed() {
    ...
    if [ -f "$AIRGAP_DIR/k3s" ] && [ -f "$AIRGAP_DIR/k3s-airgap-images-amd64.tar" ]; then
        ...
        # AB#9171: K3s's install.sh as of the pinned tag, not whatever get.k3s.io serves today.
        curl -sfL "https://raw.githubusercontent.com/k3s-io/k3s/${K3S_VERSION//+/%2B}/install.sh" | INSTALL_K3S_VERSION="$K3S_VERSION" sh -
    fi
